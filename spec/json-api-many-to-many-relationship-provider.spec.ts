...
/**
 * Let's outline the behavour a bit here.
 * M-M is usually achieved by a 1-M-1 relationship (Pivot Model)
 * That is Type A has M Pivots (P) pointing to one instance of Type B
 * Since the relationship is symmetrical, A and B are interchangeable
 * 
 * The user expected interaction here, is usually to query B from A (1-M)
 * The user may desire to add B to A (1-M)
 * 
 * 1. The User may desire to query B from A with the Pivot Data
 * 2. The User may desire to add B to A with Pivot Data
 * 3. The User may desire to query P from A with the related data (B)
 * 4. The User may desire to add P to A with the related data (B)
 * 
 * In Short, this is the same set of tests as 1-M,
 * except that we have some additional test cases involving the pivot data
 */
describe('Json api many to many relationship provider', () => 
{
